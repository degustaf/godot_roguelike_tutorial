...
func get_save_data() -> Dictionary:
	var save_data := {
		...
		"down_stairs_location": {
			"x": down_stairs_location.x,
			"y": down_stairs_location.y
		},
		"entities": [],
		"tiles": []
	}
...
func save() -> void:
	...
	var save_hash := save_string.sha256_text()
	file.store_line(save_hash)
	file.store_line(save_string)

func load_game() -> bool:
	var file = FileAccess.open("user://save_game.dat", FileAccess.READ)
	var retreived_hash := file.get_line()
	var save_string := file.get_line()
	var calculated_hash := save_string.sha256_text()
	if retreived_hash != calculated_hash:
		return false
