...
@onready var xp_bar: ProgressBar = $"%_XpBar"
@onready var xp_label: Label = $"%_XpLabel"

func initialize(player: Entity) -> void:
	if not  is_inside_tree():
		await ready
	player.level_component.xp_changed.connect(player_xp_changed)
	var player_xp := player.level_component.current_xp
	var player_max_xp := player.level_component.get_experience_to_next_level()
