...
func activate(action: ItemAction) -> bool:
	...
	var target := action.get_target_actor()
	...
	MessageLog.send_message("The eyes of the %s look vacant, as it starts to stumble around!" % target.get_entity_name(), GameColors.IMPOSSIBLE)
