...
func activate(action: ItemAction) -> bool:
	...
	var closest_distance := maximum_range + 1
	...
	for actor in map_data.get_actors():
		if actor != consumer and map_data.get_tile(actor.grid_position).is_in_view:
			var distance := consumer.distance(actor.grid_position)
	...
	if target:
		MessageLog.send_message(
			"A lightning bolt strikes %s with a loud thunder, for %d damage!" % [target.get_entity_name(), damage],
			Color.WHITE
		)
